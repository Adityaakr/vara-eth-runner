...
export default function App() {
  ...
  if (!lab.snap) {
    return (
      <div>
        <div className="mast"><div className="mark">VARA.ETH <span>Pre-confirmation · Faster UX</span></div><div className="meta"><span className="chip off"><i />{lab.status === 'open' ? 'connecting' : 'server offline'}</span></div></div>
...
export function LabView({ snap, lastError, send, request }: { snap: Snapshot; lastError: string | null; send: (cmd: object) => void; request?: Req }) {
  ...
  return (
    <div>
      <div className="mast">
        <div className="mark">VARA.ETH <span>Pre-confirmation · Faster UX</span></div>
        <div className="meta">
