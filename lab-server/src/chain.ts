...
import { ANVIL_MNEMONIC, CHAIN_ID, ETH_RPC_HTTP, FIRST_SENDER_INDEX, VARA_ETH_RPC_WS, mirrorAddress, routerAddress } from './config.js';
...
export async function connectChain(senderIndex = 0, mirrorOverride?: Address): Promise<Chain> {
  ...
  const provider = new WsVaraEthProvider(VARA_ETH_RPC_WS);
